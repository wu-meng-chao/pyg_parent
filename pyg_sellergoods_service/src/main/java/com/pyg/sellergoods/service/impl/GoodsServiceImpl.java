...
public class GoodsServiceImpl implements GoodsService {
    ...
    private void setItemValus(Goods goods, TbItem item) {
        TbGoods tbGoods = goods.getGoods();
        TbGoodsDesc tbGoodsDesc = goods.getGoodsDesc();
        item.setGoodsId(tbGoods.getId());//设置商品SPU编号
        item.setSellerId(tbGoods.getSellerId());//设置商家编号
        item.setCategoryid(tbGoods.getCategory3Id());//设置商品分类编号（3级）
        item.setCreateTime(new Date());//设置创建日期
        item.setUpdateTime(new Date());//设置修改日期
        //品牌名称
